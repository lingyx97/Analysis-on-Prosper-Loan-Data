...
Graph of each variable will be drawn, except those which do not provide any information (they will be commented out in the code).
...
The BorrowerrRate at 0.3177 may relate to some events from Q4 2011 to Q1 2013. 
...
EstimatedReturn, IncomeVerifiable and IsBorrowerHomeowner are directly related to the rating, though suprisingly, Rating and EstimatedReturn are negatively correlated;
...
What kinds of people have higher investment from friend?
...
In the multivariate (including bivariate) graph section, I said there may be relationship between delinquceies and the Prosper Rating, and also between income with prosper rating. It is reasonable to guess that delinquencies also have strong relationsip with income. So my first final graph will be as below.

```{r echo=FALSE, Final_Plot_1}
#Final Plot 1
data_final_1<-subset(data_o,!is.na(ProsperRating..numeric.)&IncomeVerifiable=='True')
data_final_1$StatedMonthlyIncome<-
  ifelse(data_final_1$StatedMonthlyIncome<=quantile(data_o$StatedMonthlyIncome,0.5),
         'below average','above average')
data_final_1$StatedMonthlyIncome<-factor(data_final_1$StatedMonthlyIncome)

ggplot(data_final_1,
       aes(color=StatedMonthlyIncome,
  ...
  xlab('ProsperRating')+
  ggtitle("Percentages of Trades That never Delinquent Against Prosper Ratings (Colored by Monthly Income)")
```

As predicted, people with higher ratings generally have lower delinquent percentages and higher income. (However, suprisingly, few people who rated 1 and 2 have pretty low y-value)
...
data_final_2$IncomeVerifiable<-
  ifelse(data_final_2$IncomeVerifiable=="True",1,0)

data_final_2$IsBorrowerHomeowner<-
  ifelse(data_final_2$IsBorrowerHomeowner=="True",1,0)

data_final_2$simple_rating_model<-
...
data_final_2<-
  subset(data_final_2,
         !is.na(data_final_2$simple_rating_model&
                  data_final_2$ProsperRating..numeric.))

ggplot(data_final_2,
       aes(factor(ProsperRating..numeric.),simple_rating_model))+
  geom_boxplot()+
  xlab("Prosper Rating")+
  ylab("Simple Rating Model")+
  ggtitle("Boxplot of A Simple Rating Model Grouped by Ratings")
```

The simple rating model is -((EstimatedReturn-min(EstimatedReturn))/(max(EstimatedReturn)-min(EstimatedReturn)))[between -1 and 0]+IsBorrowerHomeowner[0 or 1].

The two variables (in addition to income) were claimed to be related to ProsperRating, and the graph agrees with the claim except that mean model score of those rated 1 is higher than that of those rated 2 and 3. However, because people with the lowest ratings often disagree with trends in many analysis above, I would treat them as special cases (which thus do not matter) for now.

Average model scores of those rated higher than 3 are above 0, so on average, they do have house.
